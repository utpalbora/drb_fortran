...
stats = pd.DataFrame(columns=('tool', 'precision', 'recall', 'accuracy', 'F1 Score'))
...
for tool, ds in datasource.items():
    ...
    minmax = pd.merge(minraces, maxraces, how='inner', on='id', suffixes=('-min', '-max'))
    minmax['outcome'] = minmax.apply(lambda row: statsclass(row), axis=1)
    minmax.to_csv('results/'+tool+'-minmax.csv', index=False, float_format='%.0f')
    ...
    stats = stats.append({'tool': tool , 'precision': pre, 'recall': rec, 'accuracy': acc, 'F1 Score': f1} , ignore_index=True)
    TPstats = TPstats.append({'tool': tool , 'TP': TP, 'FN': FN, 'TN': TN, 'FP': FP} , ignore_index=True)
